...
export type OrgNotificationPriority = 'info' | 'success' | 'warning' | 'critical'

/** Semantic importance tier from the backend's "major events only" model. */
export type OrgNotificationLevel = 'critical' | 'important' | 'normal'
...
export interface OrgNotification {
  ...
  priority: OrgNotificationPriority
  /** Semantic importance tier. Always set by the service mapper; optional for
   * legacy payloads that predate the field. */
  level?: OrgNotificationLevel
  /** Route to navigate to when the notification is clicked. */
  actionPath?: string
  /** Raw entity link for custom handling (e.g. election id). */
  actionType?: string
  actionId?: string
}
...
export type EventStatus = 'draft' | 'ready' | 'created' | 'scheduled' | 'published' | 'live' | 'ended' | 'completed' | 'archived' | 'cancelled'
...
export interface EventCandidate {
  ...
  biography: string
  status: 'draft' | 'submitted' | 'approved' | 'rejected' | 'published' | 'withdrawn' | 'pending'
  ballotOrder: number
  voteCount: number
  createdAt: string
}
...
export interface TimelineActivity {
  ...
  description: string
  timestamp: string
  type: 'create' | 'update' | 'delete' | 'approve' | 'reject' | 'publish' | 'complete' | 'import' | 'status_change' | 'cancel' | 'alert' | 'system' | string
  user: string
}
...
export interface WorkspaceHealth {
  subscriptionStatus: 'active' | 'trialing' | 'past_due' | 'canceled' | 'inactive' | 'unavailable'
  storageUsed: number
  storageTotal: number
  activeEvents: number
  completedEvents: number
  pendingTasks: number
  notificationStatus: 'all_sent' | 'delayed' | 'failed' | 'unavailable'
