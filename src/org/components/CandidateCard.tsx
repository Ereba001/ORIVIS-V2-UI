...
import { MoreHorizontal, User, Mail, Trophy, Edit3, Trash2, Eye, FileText, GripVertical, Image } from 'lucide-react'
...
interface CandidateCardProps {
  ...
  onView?: (candidate: EventCandidate) => void
  onEdit?: (candidate: EventCandidate) => void
  onRemove?: (candidate: EventCandidate) => void
  onPhotoUpload?: (candidate: EventCandidate, file: File) => void
  onManifestoUpload?: (candidate: EventCandidate, file: File) => void
}
...
const STATUS_STYLES: Record<string, string> = {
  ...
  draft: 'bg-brand-surface-elevated text-brand-text-muted border-brand-border',
  submitted: 'bg-status-info/10 text-status-info border-status-info/20',
...
export default function CandidateCard({ candidate, positionTitle, index = 0, onReorder, onView, onEdit, onRemove, onPhotoUpload, onManifestoUpload }: CandidateCardProps) {
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div
          ...
          style={{ backgroundColor: 'var(--org-primary)', display: candidate.photoUrl ? 'none' : 'flex' }}
        >
          {initials}
        </div>
          ...
          {(candidate.party || candidate.slogan) && (
            <p className="text-[9px] font-mono text-brand-text-muted truncate mb-1">
              {candidate.partyLogoUrl && (
                <img src={candidate.partyLogoUrl} alt="" className="w-4 h-4 rounded inline mr-1 align-middle" />
              )}
              {[candidate.party, candidate.slogan].filter(Boolean).join(' · ')}
            </p>
          )}
        ...
        <div className="relative shrink-0">
          <button
            onClick={(e) => {
              if (menuOpen) { setMenuOpen(false); setMenuPos(null) }
              else {
          ...
          {/* Hidden file inputs stay inside the card so refs/clicks work */}
          <input ref={photoInputRef} type="file" accept="image/*" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) onPhotoUpload?.(candidate, f); e.target.value = '' }} />
          <input ref={manifestoInputRef} type="file" accept=".pdf,.doc,.docx" className="hidden"
            onChange={(e) => { const f = e.target.files?.[0]; if (f) onManifestoUpload?.(candidate, f); e.target.value = '' }} />
          {menuOpen && menuPos && createPortal(
            <>
              <div className="fixed inset-0 z-40" onClick={() => { setMenuOpen(false); setMenuPos(null) }} />
              <div className="fixed z-50 w-44 bg-brand-surface border border-brand-border rounded-xl py-1 shadow-xl" style={{ top: menuPos.top, right: menuPos.right }}>
                <button
                  onClick={() => { setMenuOpen(false); setMenuPos(null); onView?.(candidate) }}
                ...
                <button
                  onClick={() => { setMenuOpen(false); setMenuPos(null); photoInputRef.current?.click() }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-mono text-brand-text-muted hover:text-brand-text-primary hover:bg-brand-surface-interactive transition-colors"
                >
                  <Image size={12} />
                  Upload Photo
                ...
                <button
                  onClick={() => { setMenuOpen(false); setMenuPos(null); manifestoInputRef.current?.click() }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-mono text-brand-text-muted hover:text-brand-text-primary hover:bg-brand-surface-interactive transition-colors"
                >
                  <FileText size={12} />
                  Upload Manifesto
                ...
                <button
                  onClick={() => { setMenuOpen(false); setMenuPos(null); onEdit?.(candidate) }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-[10px] font-mono text-brand-text-muted hover:text-brand-text-primary hover:bg-brand-surface-interactive transition-colors"
                >
                  <Edit3 size={12} />
                  Edit
                ...
                <button
                  onClick={() => { setMenuOpen(false); setMenuPos(null); onRemove?.(candidate) }}
      ...
      {candidate.manifesto && (
        <p className="text-[9px] text-brand-text-muted italic truncate mt-2">{candidate.manifesto}</p>
      )}
    </motion.div>
  )
}
