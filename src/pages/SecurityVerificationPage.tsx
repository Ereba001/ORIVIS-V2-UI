...
export default function SecurityVerificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (code.some((c) => !c)) { setErrorMessage("Please enter the complete security code."); return; }
    setErrorMessage("");
  ...
  if (pageState === "success") {
    return (
      <AuthLayout title="Security Verification" subtitle="Verification completed successfully." variant="platform">
        <SeoHead meta={{ title: "Security Verification — ORIVIS", noindex: true }} />
        <AuthCard className="!shadow-none !bg-transparent !border-none !p-0 !backdrop-blur-none !max-w-none">
          <AuthStateCard
            state="success"
            title="Identity Verified"
            message="Your identity has been verified. You now have access to the requested resource."
            action={
              <button
  ...
  return (
    <AuthLayout title="Security Verification" subtitle="For your security, please enter the verification code sent to your registered device." variant="platform">
      <SeoHead meta={{ title: "Security Verification — ORIVIS", noindex: true }} />
      <AuthCard className="!shadow-none !bg-transparent !border-none !p-0 !backdrop-blur-none !max-w-none">
        {pageState === "error" && (
          <motion.div
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="text-center mb-2">
            <p className="text-[10px] text-brand-text-muted font-mono">
              A verification code has been sent to your registered email and device.
            </p>
          </div>
        ...
        <div className="flex flex-col items-center gap-2 mt-4">
          <p className="text-[10px] text-brand-text-muted/60">
            Didn't receive a code? <button className="text-brand-gold hover:underline cursor-pointer">Resend</button>
          </p>
          <button
