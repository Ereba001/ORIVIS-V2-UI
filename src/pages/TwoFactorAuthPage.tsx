...
export default function TwoFactorAuthPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (code.some((c) => !c)) { setErrorMessage("Please enter the complete verification code."); return; }
    setErrorMessage("");
  ...
  if (pageState === "success") {
    return (
      <AuthLayout title="Two-Factor Authentication" subtitle="Verification successful." variant="platform">
        <SeoHead meta={{ title: "Two-Factor Authentication — ORIVIS", noindex: true }} />
        <AuthCard className="!shadow-none !bg-transparent !border-none !p-0 !backdrop-blur-none !max-w-none">
          <AuthStateCard
            state="success"
            title="Verified"
            message="Your identity has been verified. You will be redirected to your dashboard."
            action={
              <button
  ...
  return (
    <AuthLayout title="Two-Factor Authentication" subtitle="Enter the verification code from your authenticator app." variant="platform">
      <SeoHead meta={{ title: "Two-Factor Authentication — ORIVIS", noindex: true }} />
      <AuthCard className="!shadow-none !bg-transparent !border-none !p-0 !backdrop-blur-none !max-w-none">
        {pageState === "error" && (
          <motion.div
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="flex items-center justify-center gap-2.5 text-brand-text-muted mb-2">
            <Smartphone size={16} />
            <span className="text-[10px] font-mono uppercase tracking-wider">Authenticator Code</span>
          </div>
