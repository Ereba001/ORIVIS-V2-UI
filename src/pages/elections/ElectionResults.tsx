...
import { resultsService, type ElectionResults as ElectionResultsData, type ResultsPosition } from "../../services/results-service"
      ...
      <div className="w-full bg-brand-surface-elevated py-16 relative overflow-hidden flex-grow">
        ...
        <div className="max-w-4xl mx-auto px-6 relative z-10 space-y-8">
          ...
          {resultsData?.positions.map((pos) => (
            <PositionBlock key={pos.id} position={pos} live={resultsData.live} />
          ))}
        </div>
      </div>
    </motion.main>
...
function PositionBlock({ position, live }: { position: ResultsPosition; live: boolean }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold uppercase tracking-wider text-brand-text-primary">{position.title}</h3>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-brand-text-muted font-mono">{position.totalVotes} votes</span>
          {live && (
            <span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-status-error">
              <span className="relative flex h-1.5 w-1.5">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-status-error opacity-75" />
                <span className="relative inline-flex rounded-full h-1.5 w-1.5 bg-status-error" />
              </span>
              Live
            </span>
          )}
      ...
      <div className="bg-brand-surface border border-brand-border rounded-2xl overflow-hidden">
        {sorted.map((c, i) => (
          <ResultRow key={c.id} candidate={c} index={i} totalVotes={position.totalVotes} count={count} />
        ))}
      </div>
    </div>
  )
...
function ResultRow({ candidate, index, totalVotes, count }: { candidate: ResultsPosition["candidates"][number]; index: number; totalVotes: number; count: number }) {
  const pct = totalVotes > 0 ? Math.round(((candidate.voteCount ?? 0) / totalVotes) * 100) : 0
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs font-mono text-brand-text-muted">{(candidate.voteCount ?? 0).toLocaleString()} votes</span>
