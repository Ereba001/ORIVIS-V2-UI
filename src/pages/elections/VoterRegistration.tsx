...
import type { ElectionRegistrationInfo, LookupResult, VoterLookupField, CompleteRegistrationResult } from "../../types/registration"
...
export default function VoterRegistration() {
  ...
  const [verifiedVoter, setVerifiedVoter] = useState<{ name: string; email: string } | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!info || !info.registrationEnabled || !info.registrationOpen) {
  ...
  const handleVerifyOtp = useCallback(async () => {
    ...
    try {
      ...
      if (result.voter) setVerifiedVoter(result.voter)
      // Identity verified — the voting pass is issued in the same action.
      // No separate "complete registration" step.
      try {
        const complete = await electionService.completeRegistration(registrationUuid)
        setPassResult(complete)
        setPageState("complete")
      } catch {
        // Verified but the pass could not be issued — surface the error and
        // fall back to the verified screen with a retry button.
        setOtpError("Identity verified, but the voting pass could not be issued. Please try again.")
  ...
  const handleDownloadPass = useCallback(() => {
    ...
    const text = [
      ...
      "",
      `Voting console: ${window.location.origin}/elections/${encodeURIComponent(election.slug ?? "")}/console`,
    ].join("\n")
            ...
            <div className="bg-brand-surface border border-brand-border rounded-2xl p-8 text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-brand-text-muted mt-2 max-w-sm mx-auto">
                  {regInfo?.registrationOpen === false && regInfo?.registrationEndsAt && new Date(regInfo.registrationEndsAt) < new Date()
                    ? "Registration for this election has closed."
                    : regInfo?.registrationStartsAt && new Date(regInfo.registrationStartsAt) > new Date()
                      ? "Registration for this election has not started yet."
                      : regInfo?.message ?? "Registration is not open for this election at this time."}
                </p>
              </div>
              ...
              <motion.div key="lookup" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-6">
                  <VoterRegistrationForm
                    lookupFields={regInfo?.lookupFields ?? ["name", "student_id", "staff_id"]}
              ...
              <motion.div key="preview" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-6 space-y-5">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <Detail label="Participant" value="Verified voter on record" />
                  </div>
                  {lookupResult.voter && (
                    <div className="bg-brand-bg-secondary/50 border border-brand-border rounded-xl p-4 space-y-2">
                      <p className="text-[9px] font-mono uppercase tracking-widest text-brand-text-muted font-bold">We found your record</p>
                      <Detail label="Name" value={lookupResult.voter.name} />
                      <Detail label="Registered Email" value={lookupResult.voter.email_masked} />
                    </div>
                  )}
                  <button
                  ...
                  >
                    ...
                    <span>{otpSending ? "Sending Code..." : "Verify Email & Get Voting Pass"}</span>
                  </button>
                  <p className="text-[10px] text-brand-text-muted text-center">
                    A one time code will be sent to your registered {regInfo?.verificationMethod === "email" ? "email" : "phone"}. Enter it to get your voting pass.
                  </p>
                </div>
                {otpError && !otpLocked && (
                ...
                <div className="bg-brand-surface border border-brand-border rounded-2xl p-8 text-center space-y-5">
                  ...
                  {verifiedVoter && (
                    <div className="bg-brand-bg-secondary/50 border border-brand-border rounded-xl p-4 max-w-sm mx-auto text-left space-y-2">
                      <Detail label="Name" value={verifiedVoter.name} />
                      <Detail label="Email" value={maskEmail(verifiedVoter.email)} />
                    </div>
                  )}
                  {otpError && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} role="alert" className="flex items-start gap-2 bg-status-error/10 border border-status-error/20 rounded-xl p-3 text-left">
                      <AlertTriangle size={14} className="text-status-error shrink-0 mt-0.5" />
                      <p className="text-xs text-status-error">{otpError}</p>
                    </motion.div>
                  )}
                  ...
                  >
                    ...
                    <span>{completing ? "Issuing Pass..." : "Get Voting Pass"}</span>
                  </button>
                </div>
              </motion.div>
            )}
...
function VotingPassCard({
  code, electionTitle, electionSlug, organizationName, voterName, expiresAt, onBack, onCopy, onDownload, onPrint, copied,
}: {
  code: string
  ...
  // The pass is NOT embedded in the QR/URL (it would leak via referrer and
  // browser history). The QR points to the voter console; the pass itself is
  // entered there.
  const qrValue = `${window.location.origin}/elections/${encodeURIComponent(electionSlug ?? "")}/console`
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex flex-col items-center justify-center gap-2 p-4 bg-brand-bg-secondary/30 rounded-2xl">
            ...
            <span className="text-[8px] text-brand-text-muted text-center max-w-[160px] leading-tight">
              Scan to open the voting console
            </span>
          </div>
        </div>
        ...
        <div className="bg-brand-bg-secondary/50 border border-brand-border rounded-xl p-4 space-y-1.5">
          ...
          <ol className="space-y-1">
            <li className="text-[10px] text-brand-text-muted flex items-start gap-2">
              ...
              <span>Go to the election console with your voting pass.</span>
            </li>
            <li className="text-[10px] text-brand-text-muted flex items-start gap-2">
