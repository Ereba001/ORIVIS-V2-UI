...
interface RoleCardProps {
  ...
  onViewPermissions?: (role: PlatformRole) => void
}

function RoleCard({ role, onEdit, onClone, onArchive, onUnarchive, onDelete, onViewPermissions }: RoleCardProps) {
      ...
      <div className="p-4">
        ...
        <div className="flex items-center gap-3 text-[10px]">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-mono font-bold text-brand-text-primary">{role.permissionCount}</span>
          </div>
          {!isSystem && (
            <div className="flex items-center gap-1.5">
...
export default function PlatformRoles() {
  ...
  const [viewPermsRole, setViewPermsRole] = useState<PlatformRole | null>(null)
  ...
  // Fetch permissions for the role whose permissions are being viewed
  useEffect(() => {
    if (!viewPermsRole || rolePerms[viewPermsRole.id]) return
    let cancelled = false
    platformService.getRolePermissions(viewPermsRole.id)
      .then((perms) => {
        if (!cancelled) setRolePerms((prev) => ({ ...prev, [viewPermsRole.id]: perms }))
      })
      .catch((err) => console.error('Roles.fetchPermissions:', err))
    return () => { cancelled = true }
  }, [viewPermsRole, rolePerms])
  ...
  const handleCreateOrUpdate = async () => {
    try {
      if (editingRole) {
        await platformService.updateRole(editingRole.id, {
          name: roleForm.name,
        ...
        await platformService.cloneRole(cloneSourceRole.id, { name: roleForm.name, description: roleForm.description })
        load()
      } else {
        await platformService.createRole({ name: roleForm.name, description: roleForm.description, permissions: roleForm.permissions })
        load()
      }
  ...
  const handleArchive = async () => {
    ...
    try {
      await platformService.archiveRole(archiveTarget.id)
      load()
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : 'Failed to archive role.')
    }
    setArchiveTarget(null)
  }

  const handleUnarchive = async (role: PlatformRole) => {
    try {
      await platformService.restoreRole(role.id)
      load()
    } catch (err: unknown) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await platformService.deleteRole(deleteTarget.id)
      load()
    } catch (err: unknown) {
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {systemRoles.map(role => (
              <RoleCard
                key={role.id}
                role={role}
                onViewPermissions={setViewPermsRole}
        ...
        {showCreateModal && (
          ...
          >
            <motion.div
              ...
              className="glass-strong rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto shadow-brand-lg modal-scrollbar"
            >
              ...
              <div className="p-6 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label htmlFor="roleName" className="block text-[10px] font-bold uppercase tracking-wider text-brand-text-muted mb-1.5">Role Name</label>
                    <input
                      name="roleName"
                      id="roleName"
                      value={roleForm.name}
                ...
                <div>
                  <label htmlFor="roleDescription" className="block text-[10px] font-bold uppercase tracking-wider text-brand-text-muted mb-1.5">Description</label>
                  <input
            ...
            >
              <div className="sticky top-0 bg-brand-surface border-b border-brand-border p-6 flex items-center justify-between z-10 rounded-t-2xl">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-bold text-brand-text-primary">{viewPermsRole.name} Permissions</h3>
                    <p className="text-[10px] text-brand-text-muted">
                      {viewPermsRole.permissionCount} permission{viewPermsRole.permissionCount === 1 ? "" : "s"} assigned
                    </p>
                  </div>
                </div>
                <button
              ...
              <div className="p-6">
                <PermissionMatrix
                  selectedKeys={rolePerms[viewPermsRole.id] ?? viewPermsRole.permissions}
                  allPermissions={permissions}
                />
              </div>
            </motion.div>
