...
import {
  ...
  HardDrive, Users, TrendingUp, DollarSign,
  AlertTriangle,
} from "lucide-react"
...
import type { PlatformPlan, SubscriptionRecord, SubscriptionStatus } from "../../types/platform"
...
export default function PlatformSubscriptions() {
  ...
  const load = () => {
    setLoading(true)
    setError(null)
    Promise.all([platformService.getSubscriptions({ perPage: 100 }), platformService.getPlans(), platformService.getOrganizations({ perPage: 100 })])
      .then(([subRes, planItems, orgRes]) => {
  ...
  const catalogue = useMemo<PlanInfo[]>(() => {
    const sortPlan = (a: PlatformPlan, b: PlatformPlan) => a.sortOrder - b.sortOrder
    return plans
      .slice()
      .sort(sortPlan)
      .map((p, i) => {
        const voters = p.limits?.voters ?? null
        const events = p.limits?.elections ?? null
        const priceLabel = p.isFree
          ? formatMoney(0, p.currency)
          : `${formatMoney(p.price, p.currency)}/${p.interval === "one_time" ? "once" : p.interval}`
        return {
          name: p.name,
          price: priceLabel,
  ...
  const forecast = useMemo(() => {
    ...
    const paying = records.find((s) => s.currency && s.priceValue > 0)
    const forecastCurrency = paying?.currency ?? 'NGN'
    return {
      next30Renewals: next30.length,
      next90Renewals: next90.length,
      projectedMonthly: next30.reduce((s, r) => s + r.priceValue, 0),
      projectedQuarterly: next90.reduce((s, r) => s + r.priceValue, 0),
  ...
  const selectedPlan = useMemo(() => {
    if (!selectedSub) return null
    const normalized = selectedSub.plan.toLowerCase()
    return plans.find((p) => p.slug === normalized || p.name.toLowerCase().replace(/[^a-z0-9]/g, '') === normalized) || null
  }, [selectedSub, plans])
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        >
          ...
          <p className="text-2xl font-bold font-mono text-brand-text-primary">{formatMoney(forecast.projectedQuarterly, forecast.currency)}</p>
        </motion.div>
      </div>
          ...
          <ResponsiveTable
            columns={[
              ...
              },
              {
                ...
                mobileOrder: 4,
                render: (sub) => (
                  <span className="text-xs font-mono text-brand-text-primary">{sub.maxVoters.toLocaleString()}</span>
                ),
              },
              {
                key: "price",
                ...
                mobileOrder: 5,
                render: (sub) => (
                  <span className="text-xs font-mono text-brand-text-primary">{sub.price}</span>
                ),
              },
              {
                key: "startedAt",
                ...
                mobileOrder: 6,
                render: (sub) => (
                  <span className="text-[10px] font-mono text-brand-text-muted">{formatDate(sub.startedAt)}</span>
                ),
              },
              {
                key: "expiresAt",
                ...
                mobileOrder: 7,
                render: (sub) => (
                  <span className="text-[10px] font-mono text-brand-text-muted">{formatDate(sub.expiresAt)}</span>
                ),
              },
              {
                key: "paymentMethod",
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="glass-card rounded-2xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      { label: "Max Participants", value: selectedSub.maxVoters.toLocaleString() },
                ...
                <div className="glass-card rounded-2xl p-4">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      { label: "Participants", used: selectedSub.maxVoters > 0 ? Math.round((orgHealth?.members ?? 0) / selectedSub.maxVoters * 100) : 0, usedLabel: `${(orgHealth?.members ?? 0).toLocaleString()}`, totalLabel: selectedSub.maxVoters.toLocaleString(), icon: Users },
                      { label: "Events", used: orgHealth?.activeEvents ?? 0, total: Number(selectedPlan?.limits?.elections ?? 0), usedLabel: `${orgHealth?.activeEvents ?? 0}`, totalLabel: selectedPlan?.limits?.elections == null ? "—" : String(selectedPlan.limits.elections), icon: Calendar },
                    ].map((m) => {
                      const Icon = m.icon
                      const pct = (m.total ?? 0) > 0 ? Math.min(Math.round((m.used / (m.total ?? 0)) * 100), 100) : 0
                ...
                <div className="glass-card rounded-2xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      onClick={() => {
                        if (!selectedSub || selectedSub.status === 'CANCELLED') return
                        setCancellingId(selectedSub.id)
                        platformService.cancelSubscription(selectedSub.id)
                          .then(() => {
                            setSelectedSub(null)
                            load()
                          })
                      ...
                      disabled={selectedSub.status === 'CANCELLED' || cancellingId === selectedSub.id}
                      className="flex items-center gap-1.5 px-3 py-2 bg-brand-surface-elevated hover:bg-brand-surface-interactive border border-brand-border rounded-xl text-[10px] font-mono font-bold text-brand-text-primary transition-all cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      <Ban size={12} />
                      <span>{selectedSub.status === 'CANCELLED' ? 'Cancelled' : cancellingId === selectedSub.id ? 'Cancelling…' : 'Cancel Subscription'}</span>
                    </button>
                  </div>
                </div>
              </div>
