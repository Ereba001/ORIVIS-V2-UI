...
export default function StepVerification({ data, onChange, onNext, onBack }: Props) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); onNext() }} className="space-y-4">
      ...
      <div>
        <label htmlFor="primary-color" className="block text-[10px] font-mono uppercase tracking-wider text-brand-text-muted font-bold mb-1.5">
          Primary Color
        </label>
      ...
      <div>
        <label htmlFor="secondary-color" className="block text-[10px] font-mono uppercase tracking-wider text-brand-text-muted font-bold mb-1.5">
          Secondary Color
        </label>
        <div className="flex items-center gap-3">
          <input
