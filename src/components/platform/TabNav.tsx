        ...
        return (
          <button
            ...
            className={`relative px-4 py-2 text-[11px] font-bold uppercase tracking-wider rounded-xl transition-colors cursor-pointer ${
              isActive ? "text-black" : "text-brand-text-muted hover:text-brand-text-primary"
            }`}
          >
            {isActive && (
              <motion.span
            ...
            <span className="relative z-10 flex items-center gap-2">
              {tab.label}
              {tab.count !== undefined && (
                <span className={`text-[10px] font-mono ${isActive ? "text-black/60" : "text-brand-text-disabled"}`}>
                  {tab.count}
                </span>
              )}
            </span>
